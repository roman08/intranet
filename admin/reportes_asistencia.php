...
if(isset($_GET['empleado'])){ $empleado=$_GET['empleado']; }else{ $empleado='';}
if(isset($_GET['inicial'])){ $inicial=$_GET['inicial']; }else{ $inicial=date("01/m/Y");}
if(isset($_GET['final'])){ $final=$_GET['final']; }else{ $final=date("d",(mktime(0,0,0,date("m")+1,1,date("Y"))-1)).date("/m/Y");}
?>
	<section>
		<article>
			<div class="container">
	                    	...
	                    	<div class="form-group">
		                        <ul class="listFormA">
									...
									<li class="liFecha">
										...
										<input class="form-control" readonly="readonly" type="text" name="empleado" id="empleado" placeholder="" value="<?php echo $empleado;?>">
									</li>
		                        </ul>
		                    </div>   
	  ...
	  $bus='';
	  if($empleado!=''){ $bus="and CONCAT(B.nombre, ' ', B.apellidop, ' ', B.apellidom) LIKE '%$empleado%'";}
	  ...
	  GROUP BY DATE(A.registro) and A.id_usuario 
	  order by A.registro desc 
	  limit $inicio, $rxp) X 
	  JOIN intranet_empleados B ON X.id_empleado=B.id_empleado 
	  JOIN ( SELECT DATE(A.registro) as registro, COUNT(A.registro) as accesos, MIN(A.registro) as ingreso FROM intranet_empleados_log A  where A.tipo='1' and DATE(A.registro)>='".fecha_aaaammdd($inicial)."' and DATE(A.registro)<='".fecha_aaaammdd($final)."' GROUP BY DATE(A.registro) and A.id_usuario ) Y ON X.id_empleado=Y.id_empleado and X.registro=Y.registro 
	  JOIN ( SELECT DATE(A.registro) as registro, COUNT(A.registro) as salidas, MAX(A.registro) as salida FROM intranet_empleados_log A  where A.tipo='0' and DATE(A.registro)>='".fecha_aaaammdd($inicial)."' and DATE(A.registro)<='".fecha_aaaammdd($final)."' GROUP BY DATE(A.registro) and A.id_usuario ) Z ON X.id_empleado=Z.id_empleado and X.registro=Z.registro 
	  ;");
	  
	  $total=mysql_num_rows(ProcesaQuery("select DATE(A.registro) as registro, A.id_empleado  
	  FROM intranet_empleados_log A 
	  where DATE(A.registro)>='".fecha_aaaammdd($inicial)."' and DATE(A.registro)<='".fecha_aaaammdd($final)."' $bus 
	  GROUP BY DATE(A.registro) and A.id_usuario;"));

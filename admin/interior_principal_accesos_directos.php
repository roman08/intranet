			...
			<div class="container">
				<div class="row">
					<div class="col-xs-12 col-sm-10 col-md-6">
						 <h3 class="azulMedio">Secciones Principales / Accesos directos / Agregar</h3>
					...
					<form action="admin_procesa.php" method="post" enctype="multipart/form-data" class="form-horizontal modImg" id="form_preguntas">
						<div class="col-xs-10 col-xs-offset-1 col-sm-12 col-sm-offset-0 col-md-10 col-md-offset-2" >
                            ...
                            <div class="form-group">	
								...
								<div class="col-sm-5">
									<input class="form-control" type="text" name="liga" id="liga"  required>
								</div>
							</div>
                            <div class="form-group">	
...
<section>
		<article>
			<div class="container">
				<div class="row">
					<div class="col-xs-12 col-sm-10 col-md-6">
						 <h3 class="azulMedio">Secciones Principales / Accesos directos / Modificar</h3>
						<!--<p>Registre los datos que a continuación se solicitan</p> -->
					</div>
					<div class="col-xs-12 col-sm-2 col-md-6">
					...
					<form action="admin_procesa.php" method="post" enctype="multipart/form-data" class="form-horizontal modImg" id="form_preguntas">
						<div class="col-xs-10 col-xs-offset-1 col-sm-12 col-sm-offset-0 col-md-10 col-md-offset-2" >
                            ...
                            <div class="form-group">	
								<label class="col-sm-3 control-label">* Liga</label>
								<div class="col-sm-5">
									<input class="form-control" type="text" name="liga" id="liga" value="<?php echo $row['liga'];?>" required>
								</div>
							</div>
                            <div class="form-group">	
								<label class="col-sm-3 control-label">*Icono</label>
								<div class="col-sm-5">
                                	...
                                	<?php if($row['imagen']!='' and file_exists('../accesos/'.$row['imagen'])){ echo '<img class="imagenNoticiasB" src="../accesos/'.$row['imagen'].'" /><br/>'; } else { echo 'Sin Imagen<br/>';}?>
									</div>
									<input name="archivo" id="archivo" type="file" class="form-control modIn" />
									<small>Tamaño de imagen 19 x 19px</small>
								</div>
							</div>
							<div id="contBtnDatos" class="col-sm-8 ">
                        ...
                        <input name="ver" type="hidden" id="ver" value="<?php echo $ver;?>">
                        <input name="id_acceso" type="hidden" id="id_acceso" value="<?php echo $row['id_acceso'];?>">
			...
			<div class="container">
				<div class="row">
					<div class="col-xs-12 col-sm-8">
						<h3 class="azulMedio">Secciones Principales / Accesos directos</h3>
<!-- 						<p>En la siguiente lista usted podrá visualizar las facturas que ha registrado en el sistema y el estatus de las mismas.      </p>
 -->					</div>
					<div class="col-xs-12 col-sm-4">
						...
						<button id="pregunta" class="btn btn-lg btnAzmed bco" onclick="location.href='admin.php?ver=<?php echo $ver;?>&opcion=Nuevo'"><img class="iconUser" src="img/ipreguntas.png" alt="preguntas frecuentes">Agregar Acceso</button>
					</div>
				</div>
				...
				<div id="" class="row">
					<table id="tablePreguntas" class="table table-striped table-responsive tableGeneral">
						...
						if ($inicio!='')
						  ...
						  $consulta=ProcesaQuery("select * from intranet_accesos_directos  order by orden asc limit $inicio, $rxp;");
						  $total= NumRows(ProcesaQuery("select * from intranet_accesos_directos  ;"));
						...
						if (round($total/$rxp)=='0'){$cantidad_paginas=1;}else{$cantidad_paginas=round($total/$rxp);}
						  ...
						  ?>
							<tr>
							  ...
							  <td class="centrarCelda"><?php if($row['imagen']!='' and file_exists('../accesos/'.$row['imagen'])){ echo '<img class="imagenNoticias" src="../accesos/'.$row['imagen'].'" />'; } else { echo 'Sin Imagen';}?></td>
								<td class="centrarCelda"><?php echo $row['titulo'];?></td>
                                <td class="centrarCelda"><?php echo $row['liga'];?></td>
								<td><?php if ($contador!=1) {?><a class="link_ver"  href="admin_procesa.php?ver=<? echo $ver?>&orden1=<? echo $row['orden'];?>&orden2=<? echo $row['orden']-1;?>&accion=Ordenar&inicio=<? echo $inicio;?>&id_acceso=<?php echo $row['id_acceso'];?>"><span class="glyphicon glyphicon-arrow-up naranja" aria-hidden="true" title="Subir"></span></a><?php } ?>
								<?php if ($contador!=$total)  {?><a class="link_ver"  href="admin_procesa.php?ver=<? echo $ver?>&orden1=<? echo $row['orden'];?>&orden2=<? echo $row['orden']+1;?>&accion=Ordenar&inicio=<? echo $inicio;?>&id_acceso=<?php echo $row['id_acceso'];?>"><span class="glyphicon glyphicon-arrow-down naranja" aria-hidden="true" title="Bajar"></span></a><?php } ?></td>

								<td>
                                <a class="link_ver link_verA"  href="admin.php?ver=<?php echo $ver;?>&opcion=Modificar&id_acceso=<?php echo $row['id_acceso'];?>" title="Modificar"><span class="glyphicon glyphicon-pencil naranja" aria-hidden="true" title="Editar"></span></a>
                                <a class="link_ver" href="admin_procesa.php?ver=<?php echo $ver;?>&accion=Eliminar&id_acceso=<?php echo $row['id_acceso'];?>&inicio=<?php echo $inicio;?>" title="Eliminar"><span class="glyphicon glyphicon-trash naranja" aria-hidden="true" title="Eliminar"></span></a></td>
							</tr>
                            <?php } } else {?>
							<tr>
